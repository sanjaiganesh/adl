...
declare namespace Http {
  ...
  function Path(target: any, propertyKey: string | symbol, parameterIndex: number);
  function Path(target: any, propertyKey: any, parameterIndex: any);
  function Path(target: any, parameterIndex: any);


  /** HTTP Method: PUT */
  function Put(target: any, n: string);
  function Put(target: any, propertyName: string, descriptor: TypedPropertyDescriptor<Function>);

  /** HTTP Method: DELETE */
  function Delete(target: any, n: string);
  function Delete(target: any, propertyName: string, descriptor: TypedPropertyDescriptor<Function>)

  /** HTTP Method: OPTIONS */
  function Options(target: any, n: string);
  function Options(target: any, propertyName: string, descriptor: TypedPropertyDescriptor<Function>)

  /** HTTP Method: HEAD */
  function Head(target: any, n: string);
  function Head(target: any, propertyName: string, descriptor: TypedPropertyDescriptor<Function>)

  /** HTTP Method: POST */
  function Post(target: any, n: string);
  function Post(target: any, propertyName: string, descriptor: TypedPropertyDescriptor<Function>)


  /** HTTP Method: PATCH */
  function Patch(target: any, n: string);
  function Patch(target: any, propertyName: string, descriptor: TypedPropertyDescriptor<Function>)

  /** HTTP Method: GET */
  function Get(target: any, n: string);
  function Get(target: any, propertyName: string, descriptor: TypedPropertyDescriptor<Function>)

  /** HTTP Method: TRACE */
  function Trace(target: any, n: string);
  function Trace(target: any, propertyName: string, descriptor: TypedPropertyDescriptor<Function>)

  /** A catch-all for all un-handled response codes. */
  type Default = 'default';
  type StatusCode = Http1XX | Http2XX | Http3XX | Http4XX | Http5XX | Default;

  type Http1XX = Continue | SwitchingProtocols | Processing | EarlyHints;
  type Http2XX = OK | Created | Accepted | NonAuthoritativeInformation | NoContent | ResetContent | PartialContent | MultiStatus | AlreadyReported | IMUsed | MultipleChoices;
  type Http3XX = MovedPermanently | Found | NotModified | UseProxy | SwitchProxy | TemporaryRedirect | PermanentRedirect;
  type Http4XX = BadRequest | Unauthorized | PaymentRequired | Forbidden | NotFound | MethodNotAllowed | NotAcceptable | ProxyAuthenticationRequired | RequestTimeout | Conflict | Gone | LengthRequired | PreconditionFailed | PayloadTooLarge | URITooLong | UnsupportedMediaType | RangeNotSatisfiable | ExpectationFailed | ImaTeapot | MisdirectedRequest | UnprocessableEntity | Locked | FailedDependency | TooEarly | UpgradeRequired | PreconditionRequired | TooManyRequests | RequestHeaderFieldsTooLarge | UnavailableForLegalReasons;
  type Http5XX = InternalServerError | NotImplemented | BadGateway | ServiceUnavailable | GatewayTimeout | HTTPVersionNotSupported | VariantAlsoNegotiates | InsufficientStorage | LoopDetected | NotExtended | NetworkAuthenticationRequired;
  ...
  type NonAuthoritativeInformation = 203;
  ...
  type VariantAlsoNegotiates = 506;

  /** The server is unable to store the representation needed to complete the request.  */
  type InsufficientStorage = 507;

  /** The server detected an infinite loop while processing the request (sent instead of 208 Already Reported).  */
  type LoopDetected = 508;
