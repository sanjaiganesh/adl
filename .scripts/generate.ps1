...
$outputRoot = ( resolve-path "$root/adl/test").Path
...
function run-autorest($src) {
  ...
  $txt = "$autorest --version:3.0.6185 --pipeline-model:v3 --input-file:$swaggerRoot$src --output-folder:$outputFolder --title:$name $root/adl/test/test-configuration.md $args" 
  write-host -fore GREEN "`n--------------------------------------------------------`nGenerating [$name]`n--------------------------------------------------------`n"
  echo $txt
  & $autorest "--version:3.0.6185" "--pipeline-model:v3" "--input-file:$swaggerRoot$src" "--output-folder:$outputFolder" "--clear-output-folder" "--title:$name" "$root/adl/test/test-configuration.md" "--deduplicate-inline-models" $args
  $rc = $LastExitCode
  if( $rc -gt 0 ) {
